...
You can also expose these environment variables with [vercel.json](/docs/features/configuration). For that, create a file named `vercel.json` inside your app root and add environment variables like this:
...
> It's not a good idea to commit the `vercel.json` file to [Git](https://en.wikipedia.org/wiki/Git) if it contains secret information. For that, consider using <Link href="/docs/getting-started/secrets">Secrets</Link>.
