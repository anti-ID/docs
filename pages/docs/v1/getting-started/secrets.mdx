...
If multiple people deploy your app or utilize a CI service, it's a better idea to use <Link href="/docs/getting-started/environment-variables#via-“vercel.json”">vercel.json</Link> to expose environment variables.
...
Here's how to do that with `vercel.json`:
...
This `vercel.json` file no longer contains secret information and it's safe to add that to Git and share with anyone. Only the people who can deploy the app has access to these secrets.
