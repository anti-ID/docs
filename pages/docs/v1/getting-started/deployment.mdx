...
<Note>
  ...
  <InlineCode>vercel.json</InlineCode> configuration, the Dockerfile will
  require an <InlineCode>EXPOSE</InlineCode> instruction. In this case it would
  be <InlineCode>EXPOSE 3000</InlineCode>.
</Note>
...
<Caption>
  Enabling <ProductShortV1Name /> Cloud v2 from a `vercel.json` configuration
  file.
</Caption>
