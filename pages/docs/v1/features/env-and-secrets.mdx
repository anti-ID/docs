...
If that fits your project, simply add the <Link href="/docs/features/configuration#`env`-(object)">env property</Link> to your `vercel.json` file or to the `now` property inside `package.json`:
