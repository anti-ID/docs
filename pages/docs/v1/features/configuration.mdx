...
In addition to the options available in the command line interface (like `--name` for setting the name of the deployment), we also support saving certain parameters into a config file, `vercel.json`.
...
For every type of deployment, you can create a `vercel.json` file that contains your deployment's configuration.

For example, consider a static deployment with some `.html` files in a directory called `my-website`.

Since no configuration is supplied, the deployment's name is obtained from the directory. To customize this, you can create a `vercel.json` file inside it as follows:
...
All of the properties mentioned below can be used inside the `vercel.json` file:
...
To explicitly use [version 1 of <ProductShortV1Name />](/blog/now-2), use the following configuration in a `vercel.json` file.
...
By default, they live in a directory named `.vercel` in your
