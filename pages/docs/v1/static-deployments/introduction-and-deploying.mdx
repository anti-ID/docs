...
If you want to specify a deployment as static, you can use the following configuration in your `vercel.json` file:
...
This gives the ability to preview how static apps will work, using `vercel.json` configuration or the same defaults, when deployed to <ProductShortV1Name /> without needing to deploy first, leaving no room for unexpected behavior.
