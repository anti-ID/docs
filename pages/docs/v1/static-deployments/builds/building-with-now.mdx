...
For <ProductShortV1Name /> to recognize our deployment as a static app, we'll need to mark it as a `static` type. To do this, we can add the following to the `vercel.json` configuration.
...
<Caption>
  Setting `type` as `static` in a `vercel.json` file. Read more about{' '}
...
> Please note that you can extend your `vercel.json` file as needed, but `"type": "static"` is required for static builds.

Although having a `vercel.json` is the preferred method for configuring your app deployments, you can also use the following with [<CliV1Name />](/docs/clients/now-cli):
...
If you are using routing in your SPA don't forget to add a rewriting rule to point to your `index.html`. This can be done by editing `vercel.json` and adding the following block
...
You can use a `vercel.json` configuration file and the `now secrets` command in tandem for use with the build step for static deployments.
