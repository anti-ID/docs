...
​<ProductShortV1Name /> for GitHub requires a `vercel.json` file to exist in the root of the repository to begin deploying that repository.

A `vercel.json` file is a way to configure the deployments <ProductShortV1Name /> will make using your code. To get started deploying from your repository, the `vercel.json` configuration does not need any specific setup, you will only need a valid JSON file. For example, you can use an empty object (`{}`) for the file to be valid and recognized by <ProductShortV1Name /> to start deploying.
...
​<ProductShortV1Name /> for GitHub will **deploy every push by default**. This includes pushes to any branch that includes a `vercel.json` and any pull requests made from those branches. This allows those working within the repository to preview changes made before the changes are pushed to production.

### Aliasing the Default Branch

If [an alias is set within the `vercel.json` file](/docs/features/aliases), pushes and merges to the [default branch](https://help.github.com/articles/setting-the-default-branch/) (commonly "master") will be aliased automatically and made live to those aliases with the latest deployment made with a push.

For example, the following `vercel.json` configuration will make <ProductShortV1Name /> for GitHub alias the most recent push to `example.com` by default.
...
### Disabling <NameWrapper name={PRODUCT_SHORT_V1_NAME} /> for GitHub with `vercel.json`

To disable <ProductShortV1Name /> for a GitHub repository, use the following configuration option in the `vercel.json` file:
...
<Caption>
  This example is of an entire <InlineCode>vercel.json</InlineCode> file with
  the noted configuration option to disable <ProductShortV1Name /> for GitHub.
</Caption>
...
To stop <ProductShortV1Name /> from [automatically aliasing the default branch](#aliasing-the-default-branch) to any alias setup within the `vercel.json` file, use the following configuration option.
...
<Caption>
  This example is of an entire <InlineCode>vercel.json</InlineCode> file with
  the noted configuration option to disable auto-aliasing with{' '}
...
This is a per repo setting. You can enable it by adding the following configuration option to the `vercel.json` file:
...
To stop <ProductShortV1Name /> from canceling in-progress builds to prioritise building newer commits, use the following configuration option in [a `vercel.json` file](/docs/v1/features/configuration) for the repository that this behaviour should exist in.
